...
// settings
  ...
  #define _LCDML_CONTROL_encoder_pin_a           3 // pin encoder b
  #define _LCDML_CONTROL_encoder_pin_b           2 // pin encoder a
  #define _LCDML_CONTROL_encoder_pin_button      4 // pin taster
...
void LCDML_CONTROL_setup()
{
  ...
  LCDML_BACK_dynamic_setLoopTime(LCDML_BACKEND_control, 100UL);  // 5ms 


  // init pins  
  //pinMode(_LCDML_CONTROL_encoder_pin_a      , INPUT_WITH_PULLUP);
  //pinMode(_LCDML_CONTROL_encoder_pin_b      , INPUT);
  //pinMode(_LCDML_CONTROL_encoder_pin_button , INPUT);
  //digitalWrite(_LCDML_CONTROL_encoder_pin_button, HIGH);       // turn on pullup resistors 
...
{        
  ...
  if(enter){
    unsigned int layer = LCDML.getLayer();
    if(layer < 1){
      previousLayer = layer;
    }
    drawTitle = false; 
